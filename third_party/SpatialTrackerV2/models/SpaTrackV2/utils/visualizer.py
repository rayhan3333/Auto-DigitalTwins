...
class Visualizer:
    ...
    def visualize(
        ...
        if save_video:
            self.save_frames(res_video, fg_mask)
    ...
    def save_frames(self, frames, fg_mask):
        frames = frames.squeeze(0) # (T, H, W, 3)
        fg_mask = fg_mask.squeeze().bool().unsqueeze(1)
        bg_mask = ~fg_mask.repeat(1, 3, 1, 1)
        frames[bg_mask] = 255
        frames = frames / 255
        frames = torch.cat([frames, fg_mask.float().cpu()], dim=1)
        os.makedirs(f'{self.save_dir}/track_frames', exist_ok=True)
        for i, frame in enumerate(frames):
            frame = frame.permute(1, 2, 0).cpu().numpy()
            frame = (frame * 255).astype(np.uint8)
            Image.fromarray(frame, mode='RGBA').save(f'{self.save_dir}/track_frames/{i:06d}.png')
            # torchvision.utils.save_image(frame, f'{self.save_dir}/track_frames/{i:06d}.png')
