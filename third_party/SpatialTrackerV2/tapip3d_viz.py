...
def process_point_cloud_data(npz_file, output_file, static_html_file=None, width=256, height=192, fps=4):
    ...
    try:
        ...
        trajs = np.ones((data["video"].shape[0], 10, 3))
    ...
    conf_data = data["conf"] if "conf" in data else {}
    conf_data = {}
    ...
    arrays = {
        ...
        "confs": data["conf"] if "conf" in data else None
    }

    # out_dir = os.path.dirname(npz_file)
    # viser = Visualizer(save_dir=out_dir, grayscale=True, 
    #                  fps=10, pad_value=0, tracks_leave_trace=5, show_first_frame=0)
    # trajs_torch = torch.from_numpy(trajs).cuda()
    # num_frames, num_points = trajs_torch.shape[:2]
    # intrinsics_torch = torch.from_numpy(data["intrinsics"]).cuda()
    # extrinsics_torch = torch.from_numpy(extrinsics).cuda()
    ...
    # trajs_2d[..., 0] = trajs_2d[..., 0]
    # trajs_2d[..., 1] = trajs_2d[..., 1]
    # fg_mask = torch.from_numpy(data["depths"] > 0).cuda()
    # viser.visualize(video=torch.from_numpy(data["video"][None]).cuda(),
    #                             tracks=trajs_2d[None][...,:2],
    #                             visibility=torch.from_numpy(data["visibs"][None]).cuda(),
    #                             fg_mask=fg_mask,
    #                             filename="test")
