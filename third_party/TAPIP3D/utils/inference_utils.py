...
def get_grid_queries(grid_size: int, num_points: int, depths: torch.Tensor, intrinsics: torch.Tensor, extrinsics: torch.Tensor):
    ...
    world_coords = world_coords[..., :3].to(depths.device).float()

    if num_points > 0 and world_coords.shape[1] > num_points:
        idx = farthest_point_sample(world_coords, num_points)
        world_coords = index_points(world_coords, idx)

    queries = torch.cat([torch.zeros_like(world_coords[..., :1]), world_coords], dim=-1)  # type: ignore
...
def _inference_with_grid(
    ...
    **kwargs,
    ...
    if grid_size != 0:
        additional_queries = get_grid_queries(grid_size, num_points, depths=depths, intrinsics=intrinsics, extrinsics=extrinsics)
        query_point = torch.cat([query_point, additional_queries[None]], dim=1)
