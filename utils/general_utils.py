...
def vis_depth(depth, save_path=None):
    ...
    depth = plt.get_cmap('Spectral')(depth, bytes=True)[..., :3] # [H, W, 3] [0, 255]
...
def vis_depth_numpy(depth, save_path=None):
    # depth: [1, H, W] numpy array
    depth = depth.squeeze()
    depth = depth / (depth.max() + 1e-5)
    depth = plt.get_cmap('magma')(depth, bytes=True)[..., :3] # [H, W, 3] [0, 255]
    if save_path is not None:
        Image.fromarray(depth).save(save_path)
    return depth


def dict_to_tensor(dic, device='cuda'):
    for k, v in dic.items():
        if isinstance(v,dict):
            dic[k]=dict_to_tensor(v,device)
        elif isinstance(v,np.ndarray):
            dic[k]=torch.from_numpy(v).to(device)#.to(torch.float32)
        elif torch.is_tensor(v):
            dic[k]=v.to(device)
        elif isinstance(v,list):
            raise ValueError("List not supported")
