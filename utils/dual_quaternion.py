...
def axis_angle_to_quaternion(axis: torch.Tensor, angle: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        axis: rotation axis as a tensor of shape (..., 3).
        angle: rotation angle in radians as a tensor of shape (..., 1).
    ...
    quaternion = torch.cat((cos_half_angle, axis * sin_half_angle), dim=-1)
    return quaternion


def axis2qr(axis: torch.Tensor, theta: torch.Tensor) -> torch.Tensor:
